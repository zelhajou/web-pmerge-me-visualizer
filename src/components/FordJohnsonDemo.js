    ...
    return (
      <div className="space-y-3 p-4">
        ...
        {memoryState.jacobsthal &&
          memoryState.jacobsthal.length > 0 &&
          !showJacobsthalDetail && (
            <div className="flex items-center mb-3">
              ...
              <div className="flex flex-wrap items-center">
                {memoryState.jacobsthal.map((value, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col items-center mr-1 mb-1"
                  >
                    <div
                      className={`${
            ...
            <div className="text-xs">
              <div>
                ...
                <span className="font-mono bg-blue-100 px-1 rounded">
                  {memoryState.binarySearch.value}
                </span>
              </div>
              <div className="flex flex-wrap mt-1 mb-1">
                ...
                {memoryState.binarySearch.array.map((v, i) => (
                  <span
                    key={i}
                    className="bg-gray-100 px-1 mr-1 font-mono rounded"
                  >
                    {v}
                  </span>
                ))}
  ...
  return (
    <div className="mx-auto max-w-full p-4 bg-white rounded-lg shadow-lg">
      ...
      <p className="text-gray-600 mb-4 text-sm">
        Step-by-step visualization of the merge-insert sort algorithm
      </p>
      ...
      {hasJacobsthalData && showJacobsthalDetail && (
        <div className="mt-4">
          <JacobsthalVisualization
            ...
            activeIndex={
              currentStepData.memoryState.activeIndices?.insertionOrder
            }
          />
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-md text-sm">
        ...
        <ol className="list-decimal list-inside space-y-1 mb-2">
          ...
          <li>
            Efficiently inserting the smaller elements using the Jacobsthal
            sequence
          </li>
        </ol>
        <p className="text-xs text-gray-600">
          This algorithm approaches the theoretical minimum number of
          comparisons needed for a comparison-based sorting algorithm.
        </p>
      </div>
    </div>
  );
...
function generateEnhancedSteps(inputArray) {
  ...
  const workingArray = hasStraggler
    ...
    memoryState: {
      ...
      changes: hasStraggler
        ? [
            {
              variable: "_vec",
              from: JSON.stringify(inputArray),
              to: JSON.stringify(workingArray),
            },
  ...
  steps.push({
    title: "Form Pairs",
    description:
      "Group elements into pairs from consecutive positions in the array.",
    ...
    memoryState: {
      ...
      mainChain: [],
      activeIndices: {
        vec: Array.from({ length: workingArray.length }, (_, i) => i),
      },
      operations: [
  ...
  steps.push({
    ...
    description:
      "The main chain is recursively sorted using the same algorithm.",
    ...
    memoryState: {
      ...
      activeIndices: {
        mainChain: Array.from({ length: sortedMainChain.length }, (_, i) => i),
      },
      operations: [
        ...
        `Clear _vec to prepare for result construction`,
      ],
      changes: [
        {
          variable: "mainChain",
        ...
        { variable: "_vec", from: JSON.stringify(mainChain), to: "[]" },
      ],
      level: "Recursion summary",
    },
    callStack: [
      "main()",
      "sortVector() - Main Level",
      "sortMainChainRecursively()",
    ],
  });
  ...
  steps.push({
    ...
    description:
      "Initialize the result array with the first element from the sorted main chain.",
  ...
  if (sortedPairs.length > 1) {
    ...
    steps.push({
      ...
      description:
        "Generate the Jacobsthal sequence to determine the optimal insertion order.",
    ...
    // Calculate insertion order using updated algorithm that matches your C++ implementation
    ...
    steps.push({
      ...
      description:
        "Calculate the optimal order to insert remaining elements using the Jacobsthal sequence.",
      ...
      memoryState: {
        ...
        operations: [
          ...
          `Then fill gaps in descending order between Jacobsthal numbers`,
    ...
    for (let i = 0; i < insertionOrder.length; i++) {
      ...
      // Find insertion position using lower_bound (binary search)
      let mainPos = 0;
      while (
        mainPos < currentResult.length &&
        currentResult[mainPos] < mainElement
      ) {
        mainPos++;
      }
      ...
      // Find position for smaller element using lower_bound (binary search)
      let smallerPos = 0;
      while (
        smallerPos < currentResult.length &&
        currentResult[smallerPos] < smallerElement
      ) {
        smallerPos++;
      }
  ...
  if (hasStraggler) {
    ...
    // Find position for straggler using binary search
    let pos = 0;
    ...
    steps.push({
      ...
      description:
        "Insert the straggler element (if any) at the correct position.",
  ...
  steps.push({
    title: "Final Result",
    description:
      "The array is now fully sorted using the Ford-Johnson algorithm.",
    ...
    memoryState: {
      ...
      changes: [
        ...
        {
          variable: "straggler",
          from: hasStraggler ? straggler.toString() : "null",
          to: "null",
        },
      ],
    },
